...
- (IBAction)createMatch:(id)sender {
    if ([[UserManager sharedInstance] isAvailable]){
        [self performSegueWithIdentifier:@"CreateMatch" sender:self];
    }else{
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    ...
    if ([@"DoLogin" isEqualToString:segue.identifier]){
        UINavigationController *navController = [segue destinationViewController];
        ((LoginViewController*)navController.viewControllers.lastObject).userManager = [UserManager sharedInstance];
    }else if ([@"CreateMatch" isEqualToString:segue.identifier]){
        UINavigationController *navController = [segue destinationViewController];
        ((CreateMatchViewController*)navController.viewControllers.lastObject).matchManager = [MatchManager sharedInstance];
    }
}
